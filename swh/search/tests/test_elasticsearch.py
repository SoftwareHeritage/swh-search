...
class CommonElasticsearchSearchTest(CommonSearchTest):
    """Tests shared between this module (direct ES backend test) and test_api_client.py
    ...
    def test_search_rank_forks(self):
        ...
        results = [
            r["url"]
            for r in self.search.origin_search(query='origin : "foobar.1.com"').results
        ]
        assert results == [
            "http://foobar.1.com",
            "http://foobar.2.com",
        ...
        results = [
            r["url"]
            for r in self.search.origin_search(
                query='origin : "foobar.1.com"', fork_weight=2.0
            ).results
        ]
        assert results == [
            "http://foobar.2.com",
